...
`v0.1.0-alpha.1`; its container and bundle version is `0.1.0-alpha.1` without
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\package-release.ps1 -Version 0.1.0-alpha.1
```
...
2. Create and push the annotated tag, for example
   `git tag -a v0.1.0-alpha.1 -m "Dockside v0.1.0-alpha.1"`.
